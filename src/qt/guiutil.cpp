...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ingenierobinariocoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no ingenierobinariocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ingenierobinariocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::INGENIEROBINARIOCOIN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert ingenierobinariocoin:// to ingenierobinariocoin:
    //
    //    Cannot handle this later, because ingenierobinariocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ingenierobinariocoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "ingenierobinariocoin:");
    }
...
{
    QString ret = QString("ingenierobinariocoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::INGENIEROBINARIOCOIN, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open ingenierobinariocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ingenierobinariocoin Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ingenierobinariocoin Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ingenierobinariocoin Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Ingenierobinariocoin Core*.lnk"
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ingenierobinariocoin Core\n";
        else
            optionFile << strprintf("Name=Ingenierobinariocoin Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the Ingenierobinariocoin Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Ingenierobinariocoin Core app to startup item list
