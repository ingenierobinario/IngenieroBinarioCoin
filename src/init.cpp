...
#include <config/ingenierobinariocoin-config.h>
#endif
...
public:
    ...
    // Ingenierobinariocoin Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between dashd, and ingenierobinariocoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("ingenierobinariocoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Ingenierobinariocoin Core - \""));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ingienierobinario/ingenierobinariocoin>";
    const std::string URL_WEBSITE = "<https://ingenierobinariocoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("ingenierobinariocoin-loadblk");
...
/** Sanity checks
 *  Ensure that Ingenierobinariocoin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Ingenierobinariocoin Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single Ingenierobinariocoin Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Ingenierobinariocoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Ingenierobinariocoin Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/ingienierobinario/ingenierobinariocoin/pull/1817 and https://github.com/ingienierobinario/ingenierobinariocoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Ingenierobinariocoin-specific tasks
